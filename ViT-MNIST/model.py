...
class CP_AttentionLayer(nn.Module):
    def __init__(self, rank, args):
        super(CP_AttentionLayer, self).__init__()
        self.n_attention_heads = args.n_attention_heads
        self.embed_dim = args.embed_dim
        self.head_embed_dim = self.embed_dim // self.n_attention_heads
        self.rank = rank
        # init the parameter matrix
        self.W_Q0 = nn.Parameter(torch.randn((self.embed_dim, self.rank), dtype=torch.float), requires_grad=True)
        self.W_Q1 = nn.Parameter(torch.randn((self.n_attention_heads, self.rank), dtype=torch.float), requires_grad=True)
        self.W_Q2 = nn.Parameter(torch.randn((self.head_embed_dim, self.rank), dtype=torch.float), requires_grad=True)

        self.W_K0 = nn.Parameter(torch.randn((self.embed_dim, self.rank), dtype=torch.float), requires_grad=True)
        self.W_K1 = nn.Parameter(torch.randn((self.n_attention_heads, self.rank), dtype=torch.float), requires_grad=True)
        self.W_K2 = nn.Parameter(torch.randn((self.head_embed_dim, self.rank), dtype=torch.float), requires_grad=True)

        self.W_V0 = nn.Parameter(torch.randn((self.embed_dim, self.rank), dtype=torch.float), requires_grad=True)
        self.W_V1 = nn.Parameter(torch.randn((self.n_attention_heads, self.rank), dtype=torch.float), requires_grad=True)
        self.W_V2 = nn.Parameter(torch.randn((self.head_embed_dim, self.rank), dtype=torch.float), requires_grad=True)
    ...
    def forward(self, q, k, v):
        ...
        x_queries = self.compute(q, self.W_Q0, self.W_Q1, self.W_Q2).reshape(q.shape[0], q.shape[1], self.n_attention_heads, self.head_embed_dim)  # B, Q, E -> B, Q, H, HE
        x_queries = x_queries.transpose(1, 2)  # B, Q, H, HE -> B, H, Q, HE
        x_keys = self.compute(k, self.W_K0, self.W_K1, self.W_K2).reshape(k.shape[0], k.shape[1], self.n_attention_heads, self.head_embed_dim)  # B, K, E -> B, K, H, HE
        x_keys = x_keys.transpose(1, 2)  # B, K, H, HE -> B, H, K, HE
        x_values = self.compute(v, self.W_V0, self.W_V1, self.W_V2).reshape(v.shape[0], v.shape[1], self.n_attention_heads, self.head_embed_dim)  # B, V, E -> B, V, H, HE
        x_values = x_values.transpose(1, 2)  # B, V, H, HE -> B, H, V, HE
    ...
    def compute(self, x, f0, f1, f2):
        # x in (B, S, F)
        B, S, F = x.shape
        x = x.reshape(B, -1, self.n_attention_heads, self.head_embed_dim)
        x = torch.einsum('bsnd, dr->bsnr', x, f2)
        x = torch.einsum('bsnr, nr->bsr', x, f1)
        x = torch.einsum('bsr, mr->bsm', x, f0)
...
class Encoder(nn.Module):
    def __init__(self, rank, args):
        ...
        if args.is_decompose:
            self.attention = CP_AttentionLayer(rank, args)
        else:
...
class VisionTransformer(nn.Module):
    def __init__(self, rank, args):
        ...
        self.encoder = nn.Sequential(*[Encoder(rank, args) for _ in range(args.n_layers)], nn.LayerNorm(args.embed_dim))
