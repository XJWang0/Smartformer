...
class CP_AttentionLayer(nn.Module):
    def __init__(self, rank, attention, d_model, n_heads,
                 d_keys=None, d_values=None, mix=False):
        ...
        self.W_Q2 = nn.Parameter(torch.randn((d_keys, rank), dtype=torch.float), requires_grad=True)
        ...
        self.W_K2 = nn.Parameter(torch.randn((d_keys, rank), dtype=torch.float), requires_grad=True)
        ...
        self.W_V2 = nn.Parameter(torch.randn((d_values, rank), dtype=torch.float), requires_grad=True)
