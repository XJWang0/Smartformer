...
from .utils.CP_Transformers import CP_TransformerClassifier
...
class CCT(nn.Module):
    def __init__(self,
        ...
        self.tokenizer = Tokenizer(n_input_channels=n_input_channels,
                                   ...
                                   conv_bias=False)
        if model_type == 'cp' or model_type == 'svd' or model_type == 'uniform' or model_type == 'smart':
            self.classifier = CP_TransformerClassifier(theta=theta, model_type=model_type, rank=cp_rank,
            ...
            self.classifier = TransformerClassifier(model_type=model_type, k=k, core_num=core_num,
                sequence_length=self.tokenizer.sequence_length(n_channels=n_input_channels,
                                                               height=img_size,
                                                               width=img_size),
                embedding_dim=embedding_dim,
