...
class Trans():
    def __init__(self, floder, nsub, path=1):
    ...
    def train(self):
        ...
        torch.save(self.model.module.state_dict(), './models/uniform/model.pth')
        averAcc = averAcc / num
        ...
        return bestAcc, averAcc, Y_true, Y_pred, macs, params, flops.total()
...
def step(model, encoder, action, last_averAcc, r_lim):
    reward = 0
    token = []
    real_ranks = []
    ...
    bestAcc, averAcc, Y_true, Y_pred, macs, params, flops = model.train()
    ...
    return next_info, next_state, bestAcc, averAcc, real_ranks, reward, Y_true, Y_pred, macs, params, flops




def main(seed, random_seed, epsilon):
    if random_seed:
        model_floder = f'./models/uniform/random_seed/epsilon_{epsilon}/'
        floder = f"./results/uniform/random_seed/epsilon_{epsilon}/"
    else:
        seed_n = seed
        set_seed(seed_n)
        model_floder = f'./models/uniform/seed_{seed}/epsilon_{epsilon}/'
        floder = f"./results/uniform/seed_{seed_n}/epsilon_{epsilon}/"
    ...
    # if 1:
        ...
        bestAcc, averAcc, Y_true, Y_pred, macs, params, flops = trans.train()
        best_macs, best_params, best_flops = macs, params, flops
        last_averAcc = averAcc
        
        state = arch_info.reshape(-1)
        ...
        for j in range((n_epoch - trans.n_epochs) // trans.n_epochs):
            if done:
                bestAcc_, averAcc_, Y_true, Y_pred, macs_, params_, flops_ = trans.train()
            else:
                action = ac.choose_action(state)
                next_info, next_state, bestAcc_, averAcc_, real_ranks, reward, Y_true, Y_pred, macs_, params_, flops_ = step(trans, enc, action,
                                                                                     last_averAcc, r_lim)
            ...
            if bestAcc_ > bestAcc:
                bestAcc = bestAcc_
                best_macs, best_params, best_flops = macs_, params_, flops_
        ...
        result_write.write(f'Subject {i + 1}: the rank list is {rank_list}' + "\n")
        result_write.write(f'Subject {i + 1}: the action list is {action_list}' + "\n")
        result_write.write('--------------------------------------------------------- ' + "\n")
...
def unit_time(seed):
    result_write = open(f"results/uniform/unit_time.txt", "w")

    trans = Trans(floder='results/uniform/', nsub=1, path=0)
...
if __name__ == "__main__":
    ...
    main(args.seed, args.random_seed, args.epsilon)
    # unit_time(args.seed)
