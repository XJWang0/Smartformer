...
class ParameterSharedTransformerEncoder(TransformerEncoder):
    def __init__(
        self,
        depth,
        emb_size,
        ...
        ''' since the encoder only 3 layers, we set the parameters shared in 1st and 3rd layers, ignore 2nd layer
        quotient, remainder = divmod(depth, num_unique_layers)
        assert remainder == 0
        if mode == "cycle_rev":
            assert quotient == 2
        '''
        super().__init__(num_unique_layers, emb_size)
        self.N = depth
...
class ViT(nn.Sequential):
    def __init__(self, emb_size=10, depth=3, n_classes=4, **kwargs):
        super().__init__(
            ...
            ParameterSharedTransformerEncoder(depth, emb_size),
...
class Trans():
    def __init__(self, nsub, path=1):
        ...
        self.log_write = open("results/pshared/log_subject%d.txt" % self.nSub, "w") if path else None
