...
class MultiHeadAttention(nn.Module):
    def __init__(self, seq_len, k, emb_size, num_heads, dropout):
        super().__init__()
        ...
        self.dim_head = emb_size // num_heads
    ...
    def forward(self, x: Tensor, mask: Tensor = None) -> Tensor:
        ...
        keys = self.keys(kv_input)
        values = self.values(kv_input)
    ...
    def __init__(self,seq_len, k,
        ...
        super().__init__(
            ResidualAdd(nn.Sequential(
                ...
                MultiHeadAttention(seq_len, k, emb_size, num_heads, drop_p),
                nn.Dropout(drop_p)
            )),
            ResidualAdd(nn.Sequential(
                nn.LayerNorm(emb_size),
...
class TransformerEncoder(nn.Sequential):
    def __init__(self, seq_len, k, depth, emb_size):
        super().__init__(*[TransformerEncoderBlock(seq_len, k, emb_size) for _ in range(depth)])
...
class ViT(nn.Sequential):
    def __init__(self, seq_len, k, emb_size=10, depth=3, n_classes=4, **kwargs):
        super().__init__(
            # channel_attention(),
            ResidualAdd(
                nn.Sequential(
            ...
            TransformerEncoder(seq_len, k, depth, emb_size),
...
class Trans():
    def __init__(self, seq_len, k, nsub, path=1):
        ...
        self.log_write = open(f"results/linformer/k_{k}/log_subject%d.txt" % self.nSub, "w") if path else None
...
def main(K):
    ...
    floder = f"results/linformer/k_{k}/"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--k', type=int, required=True, default=128, help='k for linformer')
    parser.add_argument('--is_train', type=int, default=1, help='train or not')
