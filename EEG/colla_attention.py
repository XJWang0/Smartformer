    ...
    def __init__(self,
        ...
        super().__init__(
            ResidualAdd(nn.Sequential(
                ...
                MultiHeadAttention(emb_size, emb_size, emb_size, emb_size, num_heads, drop_p, True),
                nn.Dropout(drop_p)
            )),
            ResidualAdd(nn.Sequential(
                nn.LayerNorm(emb_size),
...
class Trans():
    def __init__(self, nsub, path=1):
        ...
        self.log_write = open("results/colla_atten/log_subject%d.txt" % self.nSub, "w") if path else None
...
def main():
    ...
    best = 0
    aver = 0
    result_write = open("results/colla_atten/sub_result.txt", "w")
