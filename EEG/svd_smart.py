...
class MultiHeadAttention(nn.Module):
    def __init__(self, emb_size, num_heads, dropout, rank):
        super().__init__()
    ...
    def forward(self, x: Tensor, mask: Tensor = None) -> Tensor:
        ...
        # q = torch.einsum('bqf,fr->bqr', x, self.W_Q1)
        # q = torch.einsum('bqr,dr->bqd', q, self.W_Q0)
        q = torch.einsum('bqf, fr, dr->bqd', [x, self.W_Q1, self.W_Q0])

        # k = torch.einsum('bkf,fr->bkr', x, self.W_K1)
        # k = torch.einsum('bkr,dr->bkd', k, self.W_K0)
        k = torch.einsum('bkf, fr, dr->bkd', [x, self.W_K1, self.W_K0])

        # v = torch.einsum('bvf,fr->bvr', x, self.W_V1)
        # v = torch.einsum('bvr,dr->bvd', v, self.W_V0)
        v = torch.einsum('bvf, fr, dr->bvd', [x, self.W_V1, self.W_V0])
...
class TransformerEncoderBlock(nn.Sequential):
    def __init__(self, rank, emb_size,
                       num_heads=5,
                       drop_p=0.5,
                        forward_expansion=4,
                        forward_drop_p=0.5):
        ...
        attn_res = ResidualAdd(nn.Sequential(nn.LayerNorm(emb_size),
                                             MultiHeadAttention(emb_size, num_heads, drop_p, rank),
                                             nn.Dropout(drop_p)))
...
class TransformerEncoder(nn.Sequential):
    def __init__(self, rank, depth, emb_size):
        super().__init__(*[TransformerEncoderBlock(rank, emb_size) for _ in range(depth)])
...
class ViT(nn.Sequential):
    def __init__(self, rank, emb_size=10, depth=3, n_classes=4, **kwargs):
        super().__init__()
        # channel_attention(),
        ...
        transformerEncoder = TransformerEncoder(rank, depth, emb_size)
...
class Trans():
    def __init__(self,rank, floder, nsub, path=1):
    ...
    def train(self):
        ...
        torch.save(self.model.module.state_dict(), './models/svd/model.pth')
        averAcc = averAcc / num
        ...
        return bestAcc, averAcc, Y_true, Y_pred, Score, macs, params, flops.total()
...
def main(R, random_seed, seed, epsilon):
    if random_seed:
        model_floder = f'./models/svd/random_seed/epsilon_{epsilon}/'
        floder = f"./results/svd/random_seed/epsilon_{epsilon}/"
    else:
        seed_n = seed

        model_floder = f'./models/svd/seed_{seed}/epsilon_{epsilon}/'
        floder = f"./results/svd/seed_{seed_n}/epsilon_{epsilon}/"
    ...
    # if 1:
        ...
        if i == 0:
            ...
            result_write.write(
                f'Init FLOPs: {init_flops.total()}, {init_flops.total() / 1e+9} G, {init_flops.total() / 1e+6} M' + '\n')
        ...
        token_list = ['MultiHeadAttention-0-5-2-5'] * 3  # num of Attention
        ...
        result_write.write(f'Best FLOPs: {best_flops}, {best_flops / 1e+9} G, {best_flops / 1e+6} M' + '\n')
...
'''
def unit_time(R, seed):
    result_write = open(f"results/svd/seed_{seed}_R_{R}/unit_time.txt", "w")

    trans = Trans(R, floder=None, nsub=1, path=0)
...
def unit_time(R, seed, random_seed=0, epsilon=0.1):
    if random_seed:
        model_floder = f'./models/svd/random_seed/epsilon_{epsilon}/'
        floder = f"./results/svd/random_seed/epsilon_{epsilon}/"
    else:
    ...
    result_write = open(path, "w")
    trans = Trans(rank=R, floder=floder, nsub=1, path=0)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--R', type=int, default=5, help='seed')
    parser.add_argument('--epsilon', type=float, default=0.1, help='epsilon greedy')
    args = parser.parse_args()
    
    main(args.R, args.random_seed, args.seed, args.epsilon)
    # unit_time(args.R, args.seed)
    unit_time(args.R, args.seed, args.random_seed, args.epsilon)
