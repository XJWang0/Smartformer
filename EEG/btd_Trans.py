...
class MultiHeadAttention(nn.Module):
    def __init__(self, emb_size, num_heads, d_head, dropout):
        super().__init__()
        self.emb_size = emb_size
        self.num_heads = num_heads
        self.d_head = d_head if d_head is not None else emb_size//num_heads

        self.queries = nn.Linear(emb_size, num_heads * self.d_head)
        self.keys = nn.Linear(emb_size, num_heads * self.d_head)
        self.values = nn.Linear(emb_size, num_heads * self.d_head)

        self.core_nums = num_heads
        self.R = self.d_head
        self.core_value = nn.Parameter(F.softmax(torch.FloatTensor(self.core_nums, self.R), dim=-1), requires_grad=True)

        self.att_drop = nn.Dropout(dropout)
        self.projection = nn.Linear(190*190, emb_size)

    def forward(self, x: Tensor, mask: Tensor = None) -> Tensor:
        d, H = self.d_head, self.num_heads
        B, L, _ = x.size()

        queries = self.queries(x).reshape(H, B, L, -1)
        keys = self.keys(x).reshape(H, B, L, -1)
        values = self.values(x).reshape(H, B, L, -1)

        full_matrixs = 0
        for i in range(self.core_nums):
            full_matrix_1 = torch.einsum('h, bih,bjh,bkh->ibjk',
                                         [self.core_value[i], queries[i], keys[i], values[i]]).contiguous().view(B, L, -1)
        ...
        out = self.projection(full_matrixs)
    ...
    def __init__(self,
        ...
        super().__init__(
            ResidualAdd(nn.Sequential(
                ...
                MultiHeadAttention(emb_size, num_heads, d_head, drop_p),
                nn.Dropout(drop_p)
            )),
            ResidualAdd(nn.Sequential(
                nn.LayerNorm(emb_size),
...
class TransformerEncoder(nn.Sequential):
    def __init__(self, num_heads, d_head, depth, emb_size):
        super().__init__(*[TransformerEncoderBlock(emb_size, num_heads, d_head) for _ in range(depth)])
...
class ViT(nn.Sequential):
    def __init__(self, num_heads, d_head, emb_size=10, depth=3, n_classes=4, **kwargs):
        super().__init__(
            # channel_attention(),
            ResidualAdd(
                nn.Sequential(
            ...
            TransformerEncoder(num_heads, d_head, depth, emb_size),
...
class Trans():
    def __init__(self, num_heads, d_head, nsub, path=1):
        ...
        self.log_write = open(f"results/btd/head_{num_heads}_dv_{d_head}/log_subject%d.txt" % self.nSub, "w") if path else None
        ...
        self.model = ViT(num_heads, d_head).cuda()
...
def main(n_heads, d_heads):
    best = 0
    aver = 0
    num_heads = n_heads
    d_head = d_heads
    
    floder = f"results/btd/head_{num_heads}_dv_{d_head}/"
    ...
    for i in range(9):
        ...
        print('Subject %d' % (i+1))
        trans = Trans(num_heads, d_head, i + 1)
...
def unit_time(n_heads, d_heads):
    ...
    trans = Trans(num_heads, d_head, 1, 0)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_heads', type=int, required=True, default=1, help='num of core tensor')
    parser.add_argument('--d_head', type=int, required=True, default=2, help='dim of one head')
    parser.add_argument('--is_train', type=int, default=0, help='train or not')
    
    args = parser.parse_args()
    
    if args.is_train:
        main(args.num_heads, args.d_head)
        unit_time(args.num_heads, args.d_head)
    else:
        unit_time(args.num_heads, args.d_head)
