...
def get_vocabulary():
    ...
    '''encoder or decoder(0,1), n_head(1-20), dv(1-64), rank(1-450)   change them depend on your model'''
    token_list += ['MultiHeadAttention-%d-%d-%d-%d' % (i, j, k, m) for i in (0, 1)
                   for j in range(1, 11) for k in range(1, 65) for m in range(1, 701)]
...
class EncoderNet(nn.Module):
    ...
    def forward(self, token_list):
        ...
        """
        # We only need to convert characters into vectors, not take derivatives
        with torch.no_grad():
