...
log = ''
# svd 256; cp 450
token_list = ['MultiHeadAttention-0-8-64-450'] * 2
token_list += ['MultiHeadAttention-1-8-64-450']

enc = EncoderNet(512, 64)
arch_info, (h, c) = enc(token_list)

S = arch_info.reshape(-1)
...
class CP_Exp_Main(Exp_Basic):
    ...
    def compute_params_macs(self):
        ...
        macs, params = get_model_complexity_info(self.model, (1,), as_strings=True, input_constructor=self.get_input,
    ...
    def train(self, setting):
        ...
        global action_interval, action_train_loss, action_list, log, reward, state, next_state, next_info, \
            last_train_loss, action, rank_list, ac, enc, token_list, done
        training_time = []
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if action_iter % action_interval == 0 and action_iter > 1 and not done:
                    ...
                    if not done:
                        ...
                        next_info, next_state, real_rank, reward = change(self.model, action, enc, self.args.decomp_method)
                        print('-' * 100 + '\n')
                        print('| Action:{}, Rank:{}|\n'.format(action, real_rank) )
                        print('-' * 100 + '\n')
        ...
        print(log)
        print(f'The total training time is {sum(training_time):.2f}')
...
def change(model, action, enc, method):
    r = 0
    token = []
    real_ranks = []
    ...
    return next_info, next_state, real_ranks, r
