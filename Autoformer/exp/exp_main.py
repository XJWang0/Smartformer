...
score_mask = torch.tensor([1.] * 3).cuda()
done_mask = torch.tensor([1.] * 3).cuda()
action_train_loss = 0
action_interval = 100
log = ''
if not score_mask.all() == done_mask.all():
    token_list = ['MultiHeadAttention-0-8-64-440'] * 2
    token_list += ['MultiHeadAttention-1-8-64-440']
...
class CP_Exp_Main(Exp_Basic):
    ...
    def train(self, setting):
        ...
        global action_interval, action_train_loss, action_list, Score, log, reward, state, next_state, next_info, \
            last_train_loss, action, min_index, rank_list, ac, enc, token_list
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if not score_mask.all() == done_mask.all():
                    with torch.no_grad():
                        # Encoder
                        for layer in self.model.encoder.attn_layers:
                ...
                if action_iter % action_interval == 0 and action_iter > 1 and not score_mask.all() == done_mask.all():
                    ...
                    if not score_mask.all() == done_mask.all():
                        ...
                        print('-' * 100 + '\n')
                        next_info, next_state, real_rank, reward = change(self.model, action, min_index, enc)
...
def change(model, action, min_index, enc):
    r = 0
    token = []
    real_ranks = []
    ...
    for layer in model.encoder.attn_layers:
        if j == min_index:
            real_rank, r = change_cpcores(layer.attention, action, r)

        else:
            real_rank, r = change_cpcores(layer.attention, -4, r)
    ...
    for layer in model.decoder.layers:
        if j == min_index:
            real_rank, r = change_cpcores(layer.self_attention, action, r)
            real_rank, r = change_cpcores(layer.cross_attention, action, r)
            token += ['MultiHeadAttention-1-8-64-%d' % real_rank]
            real_ranks.append(real_rank)
            break

        else:
            real_rank, r = change_cpcores(layer.self_attention, -4, r)
            real_rank, r = change_cpcores(layer.cross_attention, -4, r)
            token += ['MultiHeadAttention-1-8-64-%d' % real_rank]
            real_ranks.append(real_rank)
