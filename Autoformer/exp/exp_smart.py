...
class CP_Exp_Main(Exp_Basic):
    ...
    def train(self, setting):
        ...
        global done, Score, action_list, rank_list, state, next_state, next_info, min_index, action, reward, last_train_loss, action_train_loss, action_interval, log
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if action_iter % action_interval == 0 and action_iter > 1 and not self.frame.score_mask.all() == self.frame.done_mask.all():
                    ...
                    if action_step > 0:
                        ...
                        self.frame.ac.train_net(state, reward, next_state, action)
                        state = next_state
                        last_train_loss = ll
                    ...
                    if not self.frame.score_mask.all() == self.frame.done_mask.all():
                        ...
                        next_info, next_state, real_rank, reward = change(self.model, action, min_index, self.frame.enc, self.args.decomp_method)
                        print('-' * 100 + '\n')
